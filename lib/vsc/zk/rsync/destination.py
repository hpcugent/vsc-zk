...
class RsyncDestination(RsyncController):
    ...
    def __init__(self, hosts, session=None, name=None, default_acl=None,
                 auth_data=None, rsyncpath=None, rsyncport=None, startport=4444,  # root default rsyncport: 873
    ...
    def run_netcat(self, port):
        """ Test run with netcat """
        return self.run_with_watch('nc -l -k %s' % port)
    ...
    def run(self, attempts=3):
        ...
        while (attempt <= attempts and not self.is_ready()):
            ...
            self.dest_queue.put('%s:%s' % (port, self.whoami))
            if self.netcat:
                self.run_netcat(port)
            else:
                self.run_rsync(port)
